...
router.post( '/create_user',
	...
	wrapAsync( async function onCreateUser( req, res ) {
		...
		if ( settings.reservedUserNames && settings.reservedUserNames.length > 0 ) {
			if ( req.body.name && contains( settings.reservedUserNames, req.body.name ) ) {
				throw new ErrorStatus( 400, req.t('entered-reserved-username') );
			}
			if ( req.body.preferredName && contains( settings.reservedUserNames, req.body.preferredName ) ) {
				throw new ErrorStatus( 400, req.t('entered-reserved-username') );
			}
		}
		...
		try {
			...
			if ( req.body.firstName || req.body.lastName || req.body.preferredName ) {
				validateString( req.body.firstName, 'firstName', req.t );
				validateString( req.body.lastName, 'lastName', req.t );
				userConfig.firstName = req.body.firstName;
				userConfig.lastName = req.body.lastName;
			}
			else {
				validateString( req.body.name, 'name', req.t );
				userConfig.name = req.body.name;
			}
 ...
 *               name:
 *                 type: string
 *                 description: User's new name (individual parts take precedence)
 *                 example: John Doe
 *               firstName:
 *                 type: string
 ...
 *               preferredName:
 *                 type: string
 *                 description: User's new preferred name
 *                 example: John
 *               pronouns:
 *                 type: string
...
router.post( '/update_user',
	...
	wrapAsync( async function onUpdateUser( req, res ) {
		...
		const hasPreferredName = isString( req.body.preferredName );
		if ( req.body.firstName || req.body.lastName || hasPreferredName ) {
