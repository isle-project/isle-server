 ...
 *           assessments:
 *             type: object
 *             description: string keys constructed from level-entityId-metric.name, values are sub-objects with keys metricName, lastUpdated, instance. The instance component in turn has the form {level, entity, score, time, provenance, tag}, provenance is either null (for component assessments) or an array of instances of child assessments, and tag is present only if not DEFAULT_TAG
 *             example: {"metricName": "homeworkGrade", lastUpdated: "2022-04-28+2230.1425Z", instance: { "level": "program", "entityId": <objectID>, "score": 10, "time": 1660014707303, provenance: [{"level": "namespace", "entity": <objectID1>, "score": 5, time: 1660014704300, provenance: [], tag: "homework2" }] } }
 *             default: {}
 *           badges:
 *             type: Array
...
UserSchema.pre( 'save', function save( next ) {
	...
	function onSalt( err, salt ) {
		...
		debug( 'Hash the password along with our new salt...' );
		bcrypt.hash( user.password, salt, function hashpw( err, hash ) {
			if ( err ) {
				return next( err );
			}

			debug( 'Override the cleartext password with the hashed one...' );
			user.password = hash;
			next();
		});
	}
