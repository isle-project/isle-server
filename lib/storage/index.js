...
const logger = require( './../debug' )( 'server:storage' );
...
const debug = logger( 'isle-server:storage' );
...
const MEDIA_BRANDING_DIR = join( MEDIA_BASE_DIR, 'branding' );
const FILES_DIRECTORY = join( MEDIA_DIRECTORY, 'files' );


// FUNCTIONS //

/**
 * Determines the destination for a file upload.
 *
 * @private
 * @param {Object} req - express request object
 * @param {Object} file - express file object
 * @param {Function} cb - callback to invoke after determining destination with error and destination path parameters
 * @returns {void}
 */
function onDestination( req, file, cb ) {
	...
	if ( file.fieldname === 'branding' ) {
		return cb( null, MEDIA_BRANDING_DIR );
	}
	return cb( null, MEDIA_DIRECTORY );
}

/**
 * Determines the filename for a file upload.
 *
 * @private
 * @param {Object} req - express request object
 * @param {Object} file - express file object
 * @param {Function} cb - callback to invoke after determining filename with error and filename parameters
 * @returns {void}
 */
function onFilename( req, file, cb ) {
...
debug( 'Directory to store files in: '+FILES_DIRECTORY );
