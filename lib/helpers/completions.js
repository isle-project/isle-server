 ...
 * ============
 *
 * The public entry point for computing completion results at every level
 * is @see {@link computeCompletions}.
 *
 ...
 *   + Tag weights to use in aggregating instances
 *   + A multiples policy specifying how to handle multiple
 *         submissions for one component.
 *
 * @type {Object}
 * @property {Array<number>} timeFilter
 * @property {Object<string,number>|undefined} tagWeights
...
const maxInstanceTime = (maxTime, instance) => {
    ...
    return instance[1] >= maxTime ? instance[1] : maxTime;
};
 ...
 *
 * @private
 * @param {Array} source - an iterable
 * @param {function} accessor - given an element of source, returns the
 *     corresponding tag string; if null is returned, this element is
 *     ignored. If accessor returns an array, all its elements are added
 *     to the tag set.
 * @param {Object<string,number>} weights - an object specifying tag
 *     weights whose keys are used to initialize the set
...
function makeTagSet( source, accessor, weights, addDefault ) {
    ...
    debug( 'Initial Tag Set', Array.from(tagSet) );
    ...
    debug( 'Final Tag Set', Array.from(tagSet) );
    return tagSet;
}
 ...
 * @private
 * @param {Array<Instance>} arr - array of completion instances to summarize
 * @param {('zero'|'ignore')} missing - how to handle missing values;
 *     either 'zero' (impute zero value) or 'ignore' (drop from calculation).
 *
 * @returns {number} proportion of trues among scores.
 */

function binaryProportion( arr, missing = 'zero' ) {
    const toBinary = x => x >= 50 ? 100 : 0;
    let scores;
    if ( missing === 'ignore' ) {
        scores = arr.filter(notMissingScore).map( x => toBinary(x[0]) );
    } else {
        scores = arr.map( x => toBinary(imputeMissingScore(x[0])) );
    }
 ...
 *
 * @returns {number} average of the decayed scores, ignoring missing values.
 */
...
function decayedAverage( arr, deadline, halving, cap = Infinity ) {
    ...
    const instances = arr.filter(notMissingScore);
...
/**
 * Query the Completion database table, sorting results by time.
 ...
 *
 * @returns Promise<Array<Object>> array of full Completion documents matching
 *     query and sorted by recorded time.
 */

async function getMatchingCompletions( query, order = 'ascending' ) {
    return await Completion.find( query ).sort({ 'time': order });
}

/**
 * For a set of branch nodes, returns associated completion metrics and tags.
 *
 * @private
 ...
 *     completions (array of metrics), and tag keys associated with the specified nodes.
 */

async function getBranchMetricsAndTags( schema, nodes ) {
    return await schema.find({ _id: { $in: nodes } }, { completions: 1, tag: 1 });
}
...
async function getBranchData(metric, nodes, level, users, policyOptions) {
    ...
    const completionData = await getBranchMetricsAndTags( schema, nodes );
    if ( completionData.length === 0 ) {
        return [];  // ATTN:? when empty, when MISSING?
    }
    ...
    if ( objectKeys(submetrics).length === 0 ) {
        return []; // ATTN:? when empty, when MISSING? ; ATTN:CHANGED from throw - check downstream handling
    }
    ...
    const children = await Promise.all(nodes
                                       ...
                                       .map(node => Promise.all([ Promise.resolve(tags[node]),
                                                                  computeCompletions(node, submetrics[node], users, policyOptions) ])) );
    const branchData = [];
    const tagSet = makeTagSet( children, first, metric.tagWeights || policyOptions.tagWeights );

    // Distribute child aggregate instances into proper TaggedUserCompletions shape
    for ( let i = 0; i < children.length; ++i ) {
        const thisBranch = {};
...
async function getComponentData( metricName, nodes, _, users, policy ) {
    ...
    const records = await getMatchingCompletions( query, timeOrder );
    debug( 'Completion Instances', records );
    if ( records.length === 0 ) {
        return []; // ATTN:? when empty, when MISSING? ; ATTN:CHANGED from throw - check downstream handling
    }
    ...
    if ( policy.multiples === 'last' || policy.multiples === 'first' ) {
        for ( let i = 0; i < records.length; i++ ) {
            ...
            nodeCompletions[ component ][ xtag ][ user ] = [ [score, time, {...provenance, entity: component }] ];
        ...
        for ( let i = 0; i < records.length; i++ ) {
            ...
            if ( completion.length === 0 || completion[0][0] < score ) {
                completion = [ [score, time, {...provenance, entity: component }] ];
            }
            nodeTags[ component ][ xtag ] = nodeTags[ component ]?.[ xtag ] ? ++nodeTags[ component ][ xtag ]: 1;
        }
        ...
        for ( let i = 0; i < records.length; i++ ) {
            const {component, user, score, time, tag} = records[ i ];
            const xtag = tag || DEFAULT_TAG;
            nodeCompletions[ component ][ xtag ][ user ].push( [score, time, {...provenance, entity: component }] );
            nodeTags[ component ][ xtag ] = nodeTags[ component ]?.[ xtag ] ? ++nodeTags[ component ][ xtag ]: 1;
        }
    }
    ...
    for ( const component of nodes ) {
        ...
        for ( const user of users ) {
            if ( myTags.every( tag => nodeCompletions[ component ][ tag ][ user ].length === 0 ) ) {
                nodeCompletions[ component ][ myTag ][ user ].push( MISSING );
            }
        }
    }
...
function groupCompletions( nodeCompletions, users, tagWeights ) {
    ...
    const tagSet = makeTagSet( nodeCompletions, c => Object.keys(c), tagWeights );
...
function weightCompletions( reduced, level, entity, users, tagWeights ) {
    const computed = {};
    if ( tagWeights ) {
        for ( const userId of users ) {
            ...
            for ( const tag in reduced ) {
                ...
                if ( reduced[ tag ]?.[ userId ] !== void 0 ) {
                    ...
                    const [score, _, provenance ] = instance;
                    total += score * weight;
            ...
            const avgScore = ( weightTotal > 0 ) ? total / weightTotal : 0;  // ATTN:? MISSING instead of 0?
...
async function gatherCompletions( id, metric, users, policyOptions ) {
    ...
    debug( `Branch data from relevant children for ${metric.level} ${id}`, nodeCompletions );
    ...
    debug( `Completions by users in gather completions at ${metric.level}`, taggedCompletions );
    return taggedCompletions;
}
 ...
 *
 *     This contains for each an instance [<score>, <time>, <provenance>] that
 *     aggregates the completion data at the subtree rooted at `id`. The score
 *     is determined by the metric rule; the time is the maximum time over the
 *     subtree, and the provenance gives a tree of all the data that went into
 *     this calculation.
 *
 */

async function computeCompletions( id, metric, users, policyOptions ) {
    const tagWeights = metric.tagWeights || policyOptions.tagWeights;
    ...
    const aggregate = weightCompletions( reduced, metric.level, id, users, tagWeights );

    debug( `computeCompletions gathered  at ${metric.level} ${id}`, completions );
    debug( `computeCompletions reduced   at ${metric.level} ${id}`, reduced );
    debug( `computeCompletions aggregate at ${metric.level} ${id}`, aggregate );
