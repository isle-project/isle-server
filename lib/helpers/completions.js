...
/**
 * Mapping from a node level to the matching schema and its field containing the children.
 * If the schema field is missing, the child data are located elsewhere.
 * For example, component data is stored in a separate table.
 */
const levelMapping = {
        program: {
                schema: Program,
...
/**
 * Returns an array of node id's for those that match the coverage criteria and level.
 *
 * @private
...
/**
 * Returns an array of objects, with each mapping user ids to completion data.
 *
 * @private
 ...
 * @returns {Promise} resolves to an array of objects, each mapping user ids to
 *     completion data (i.e., arrays of value-time pairs). Specifically, the
...
/**
 * Returns an array of objects, each mapping user ids to tags to completion data.
 * 
 * Each completion data instance is a pair of a value (number from
 * 0-100) and a time (possibly undefined). Only instances that are
 * within the policy's time window are kept and returned.
 *
 * @private
 ...
 *
 * @returns {Promise<[Object]>} resolves to an array of objects, each mapping
 *     user ids to tags to completion data (i.e., arrays of value-time pairs).
...
async function getLeafData( ref, nodes, level, users, policy ) {
    ...
    //
    // Note: We accept an extra pass through instances here for simplicity.
    ...
    // Ensure that missing components for a user are marked with 0
    for ( const component of nodes ) {
        ...
        for ( const user of users ) {
            if ( objectValues(nodeCompletions[ component ][ user ]).every( v => v.length === 0 ) ) {
                nodeCompletions[ component ][ user ][ myTag ].push( [0, void 0 ] );
            }
        }
    }
...
/**
 * Groups completion data into a single object mapping keyed by user ids.
 * The associated value is itself an object mapping tags to arrays of
 * value-time pairs.
 *
 ...
 *
 * @private
 * @param {Array} nodeCompletions - array of objects mapping user ids to completion
 *     data (i.e., array of value-time pairs). Specifically, this parameter has shape:
 *         [
 *             {
 *                 [userId]: {
 *                     [tag]: [[<0-100>, <time|void 0>], ...],
 ...
 *     (@see {@link makeCompletionPolicy}).
 * @returns {Object} object mapping user ids to tags to an array of two-element
 *     arrays of completion values and time. Specifically, the returned object
 *     looks like:
 *         {
 *             [userId]: {
 *                 [tag]: [ [<0-100>, <time|void 0>], ... ],
...
/**
 * Transformed a grouped completions object to give a single score for each user and tag.
 * Returns an object mapping users ids to tags to aggregate
 * completion value for the given metric.
 *
 ...
 *     been merged into DEFAULT_POLICY so that all fields are available
 *     (@see {@link makeCompletionPolicy}).
 * @returns {Promise<Object>} object mapping user ids to completion data for the
