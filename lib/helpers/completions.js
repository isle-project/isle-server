...
const levelMapping = {
	...
	lesson: {
		schema: Lesson,
		field: 'components' // TODO: revisit whether to use this
	},
	component: {
		schema: Completion
	}
...
async function relevantNodes( id, level, coverage, users ) {
	...
	if ( mapping.field ) {
		...
		children = await Completion
			.find({ lesson: id, user: { $in: users } }, { component: 1 } )
			.distinct( 'component' );
	}
	if ( coverage[ 0 ] === 'all' ) {
		return children;
...
async function getBranchData( ref, nodes, lowerLevel, users ) {
	const { schema } = levelMapping[ lowerLevel ]; // TODO: handle component level
...
* @param {null} lowerLevel - lower level of the hierarchy, not used for component level
* @param {Array<string>} users - array of user ids
* @returns {Promise} - array of objects mapping user ids to completion data
*/
async function getLeafData( ref, nodes, lowerLevel, users ) {
...
// gatherCompletions( namespaceId, metric { level: 'namespace' }, users, getBranchData );


function gatherCompletions( id, metric, users, getData ) {
	// Case: course level
	const nodes = relevantNodes( id, metric.level, metric.coverage, users );
