...
const COMPLETION_RULES  = require( './completion_rules.json' );
...
async function getBranchData(metric, nodes, level, users, policyOptions) {
    ...
    const children = await Promise.all(nodes
                                       ...
                                       .map(node => Promise.all([
                                           ...
                                           computeCompletions(String(node), submetrics[node], users, policyOptions) // Note: Can accept ObjectId, using String to match expected type
                                       ])) );
    const branchData = [];
 ...
 *
 * @returns {ReducedCompletions} aggregate instance for each tag and user pair,
...
module.exports = {
    ...
    isMissingScore,        // Check if a completion score represents a missing
    COMPLETION_RULES,      // Map of rule names to rule specifications
