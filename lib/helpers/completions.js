...
//  8. Remove score and time from outer object (but keep metricName) since they're already in instance
...
function joinProvenances( base, sibling ) {
    if ( !isUndefinedOrNull( sibling.provenance ) ) {
        if ( base.provenance ) {
            base.provenance = base.provenance.concat( sibling.children );
        } else {
...
function makeTagSet( source, accessor, weights, addDefault ) {
    const tagSet = new Set( weights ? objectKeys(weights) : [] );
...
async function getComponentData( metricName, nodes, _, users, policy ) {
    ...
    if ( policy.multiples === 'last' || policy.multiples === 'first' ) {
        for ( let i = 0; i < records.length; i++ ) {
            ...
            nodeCompletions[ component ][ xtag ][ user ] = [ instance ];
        ...
        for ( let i = 0; i < records.length; i++ ) {
            ...
            if ( completion.length === 0 || completion[0].score < score ) {
                const instance = makeInstance( level, component, score, time, null, xtag );
                nodeCompletions[ component ][ xtag ][ user ] = [ instance ];
            }
            nodeTags[ component ][ xtag ] = nodeTags[ component ]?.[ xtag ] ? ++nodeTags[ component ][ xtag ]: 1;
        ...
        for ( let i = 0; i < records.length; i++ ) {
            ...
            const xtag = tag || DEFAULT_TAG;
            const instance = makeInstance( level, component, score, time, null, xtag );
            nodeCompletions[ component ][ xtag ][ user ].push( instance );
            nodeTags[ component ][ xtag ] = nodeTags[ component ]?.[ xtag ] ? ++nodeTags[ component ][ xtag ]: 1;
        }
    }
    ...
    for ( const component of nodes ) {
        ...
        for ( const user of users ) {
            if ( myTags.every( tag => nodeCompletions[ component ][ tag ][ user ].length === 0 ) ) {
                nodeCompletions[ component ][ myTag ][ user ].push( missingInstance( level, component, myTag ) );
            }
        }
    }
...
function groupCompletions( nodeCompletions, users, tagWeights ) {
    ...
    const tagSet = makeTagSet( nodeCompletions, c => objectKeys(c), tagWeights );
...
function weightCompletions( reduced, level, entity, users, tagWeights ) {
    // If no positive weights given, all extant tags weighted equally, which could be
    // only DEFAULT_TAG. Otherwise, any missing weights in tagWeights are taken as 0.
    const defaultWeight = (tagWeights && objectValues(tagWeights).some(isPositiveNumber)) ? 0 : 1;
    ...
    for ( const userId of users ) {
        ...
        for ( const tag in reduced ) {
            ...
            if ( !isUndefinedOrNull( instance ) ) {
                if ( isNotMissing( instance ) ) {    // Missing parts imputed to zero
...
async function gatherCompletions( id, metric, users, policyOptions ) {
    ...
    if ( nodeCompletions.length > 0 ) {
        taggedCompletions = groupCompletions( nodeCompletions, users, metric.tagWeights );
    } else {
...
async function computeCompletions( id, metric, users, policyOptions ) {
    ...
    const aggregate = weightCompletions( reduced, metric.level, id, users, metric.tagWeights );
