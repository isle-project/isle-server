...
*
* @private
* @param {Array} arr - two-element array of value and time pairs
...
* @private
* @param {Array} arr - two-element array of value and time pairs
* @returns {number} average after dropping lowest score
*/
...
async function relevantNodes( id, level, coverage, users ) {
	...
	console.log( 'Relevant nodes:', JSON.stringify( out ) );
	return out;
}
 ...
 */
async function getBranchData( metric, nodes, level, users ) {
	...
	if ( level === 'component' ) {
		...
		return getLeafData( metric.ref, nodes, null, users, metric.multiplesPolicy );
	}
	...
	if ( completions.length === 0 ) {
		console.log( 'No completions found, returning empty array.' );
		return [];
	}
...
* @param {string} multiplesPolicy - the policy for handling multiple completion data for the same user (either `first`, `last`, `max`, or `pass-through`)
* @returns {Promise} resolves to an array of objects mapping user ids to completion data
*/
async function getLeafData( ref, nodes, level, users, multiplesPolicy = 'last' ) {
	...
	const completions = await Completion
		.find( query )
		.sort({ 'time': multiplesPolicy === 'last' ? 'ascending' : 'descending' });
	console.log( 'Completions: ', JSON.stringify( completions ) );

	const nodeCompletions = {};
	const extras = [];
	for ( let i = 0; i < nodes.length; i++ ) {
		nodeCompletions[ nodes[ i ] ] = {};
		for ( let j = 0; j < users.length; j++ ) {
			nodeCompletions[ nodes[ i ] ][ users[ j ] ] = {
				value: 0,
				time: void 0,
				tag: void 0
			};
	...
	if ( multiplesPolicy === 'last' || multiplesPolicy === 'first' ) {
		for ( let i = 0; i < completions.length; i++ ) {
			const completion = completions[ i ];
			nodeCompletions[ completion.component ][ completion.user ] = {
	...
	else if ( multiplesPolicy === 'max' ) {
		for ( let i = 0; i < completions.length; i++ ) {
			const completion = completions[ i ];
			const existingValue = nodeCompletions[ completion.component ][ completion.user ].value;
			if ( existingValue < completion.value ) {
				nodeCompletions[ completion.component ][ completion.user ] = {
					value: completion.value,
					time: completion.time,
					tag: completion.tag
		...
		for ( let i = 0; i < completions.length; i++ ) {
			const completion = completions[ i ];
			const seenKey = `${completion.component}-${completion.user}`;
			const val = {
				value: completion.value,
				time: completion.time,
				tag: completion.tag
			...
			if ( !seen[ seenKey ] ) {
				nodeCompletions[ completion.component ][ completion.user ] = val;
			} else {
				extras.push({
					[completion.user]: val
				});
	...
	return nodes.map( x => nodeCompletions[ x ] ).concat( extras );
}
...
async function gatherCompletions( id, metric, users ) {
	...
	let nodeCompletions = await getBranchData( metric, nodes, predecessor( metric.level ), users );
	if ( metric.level === 'namespace' ) {
		console.log( 'Node completions at namespace level: ', JSON.stringify( nodeCompletions ) );
	}
	/*
		[
			{
	...
	*/
	if ( nodeCompletions.length > 0 ) {
		...
		console.log( `Completions by users in gather completions at ${metric.level}:`, JSON.stringify( byUsers ) );
		return reduceCompletions( byUsers, metric );
	}
	// Case: no completion data for the given metric and users:
	const byUsers = {};
