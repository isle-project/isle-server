...
const { computeAssessments } = require( './assessments.js' );
 ...
 *    - computeAssessments for metric for user
...
function makeAssessmentDependencies( componentMetric, lesson, namespace ) {
	...
	const lessonMetrics = lesson.assessments.reduce( (map, x) => {
		if ( x.submetric === componentMetric ) {
			map.set( x.name, x );
		}
		return map;
	}, new Map() );
	const namespaceMetrics = namespace.assessments.reduce( (map, x) => {
		if ( x.autoCompute && lessonMetrics.has( x.submetric ) && isCoveredLesson( lesson._id, x.coverage ) ) {
			if ( map.has( x.submetric ) ) {
				map.get( x.submetric ).push( x );
			} else {
				map.set( x.submetric, [ x ] );
			}
		}
		return map;
	}, new Map() );
...
async function updateAutoComputes( cache, user, componentMetric, lessonId, namespaceId ) {
	...
	for ( let i = 0; i < computations.length; ++i ) {
		...
		const assessment = await computeAssessments( id, metric, [ user._id ], {
			activeComponents: cache.activeComponents
		} );
		const instance = assessment[ user._id ];

		// Store main assessment:
		const mainKey = `${metric.level}-${id}-${metric.name}`;
		user.assessments[ mainKey ] = { metricName: metric.name, lastUpdated, instance };
		...
		if ( derivedId !== void 0 ) {
			...
			user.assessments[ derivedKey ] = { metricName: derivedMetric.name, lastUpdated, instance: derivedInstance };
		}
	}
...
function referringAutoComputedMetrics( cache, namespaceId, lessonId, lessonMetric ) {
	const subtree = cache.forest?.[ lessonId ]?.[ lessonMetric.submetric ];
	const out = [];
	if ( !subtree ) {
		return out;
	}
	for ( const [ id, metric ] of subtree ) {
		if ( isEqualObjectId( namespaceId, id ) && metric.autoCompute && metric.submetric === lessonMetric.name ) {
			out.push( metric );
		}
	}
	return out;
	...
	if ( assessmentDepsCacheNeedsUpdate( cache, metric.level, entityId, metric.name, metric.autoCompute, hasReferringAutoCompute ) ) {
		if ( isLessonLevel ) {
			...
			if ( metric.autoCompute && hasReferringAutoCompute ) {
				...
				const nodes = ensureNode( cache, entityId, metric.submetric );
			...
			lessons.forEach( lesson => {
				...
				assessments
					.filter( x => x.name === metric.submetric )
					.forEach( lessonMetric => {
						setAssessmentDependencies( cache, lessonMetric.submetric, lesson, namespace );
					});
			});
