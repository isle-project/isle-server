...
async function getMatchingAssessments( query, order = 'ascending' ) {
    const res = await Assessment.find( query ).sort({ 'time': order });
	return res;
}
...
async function getBranchMetricsAndTags( schema, nodes ) {
    const res = await schema.find({ _id: { $in: nodes } }, { assessments: 1, tag: 1 });
	return res;
}
...
async function getBranchData(metric, nodes, level, users, policyOptions) {
    if ( level === 'component' ) {
        ...
        return getComponentData(metric.submetric, nodes, null, users, policy);
    }
    // The next lower level is a namespace or lesson
    // Collect assessment metrics that match the specified submetric (or first available if submetric null)
    ...
    const submetric = metric.submetric;
...
async function getComponentData( metricName, nodes, _, users, policy ) {
    ...
    for ( const component of nodes ) {
        ...
        const isComponentMetricActive = policy.activeComponents && policy.activeComponents.has( `${metricName}-${component}-${lessonID}`);
        if ( isComponentMetricActive ) {
            for ( const user of users ) {
