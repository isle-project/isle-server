...
async function getPopulatedNamespace( namespaceId ) {
	const namespace = await Namespace.findById( namespaceId );
	await namespace.populate( 'lessons' ).execPopulate();
	return namespace;
}
...
function setAssessmentDependencies( cache, componentMetric, lesson, namespace ) {
	...
	if ( !hasOwnProp( cache.forest, namespaceId ) ) {
		cache.forest[ namespaceId ] = {};
	}
	cache.forest[ lessonId ][ componentMetric ] = computationSet;  // Explicitly shared, only need to modify one!
	cache.forest[ namespaceId ][ componentMetric ] = computationSet;
	return computationSet;
}
 ...
 */
 function removeAssessmentDependencies( cache, level, entityId, metricName ) {
	const subtree = cache.forest[ entityId ];
	if ( !subtree ) {
		return cache;
	}
	if ( level === 'namespace' ) {
			...
			if ( hasOwnProp( subtree, componentMetric ) ) {
				...
				if ( affectedNodes ) {
					for ( let i = affectedNodes.length - 1; i >= 0; --i ) {
						...
						const isRelevant = nsId === entityId && nsMetric.name === metricName;
			...
			if ( hasOwnProp( subtree, componentMetric ) ) {
				...
				if ( affectedNodes ) {
					...
					for ( let i = affectedNodes.length - 1; i >= 0; --i ) {
						...
						if ( idA === entityId && metricA.name === metricName ) {
							affectedNodes.splice( i, 1 );
						} else if ( idB === entityId && metricB.name === metricName ) {
 ...
 * @param {DepsCache} cache - cache object
 * @param {ObjectId} namespaceId - namespace id
 * @param {string} lessonMetricName - lesson metric name
 * @returns {Array<Object>} array of auto-computed namespace metrics using the given lesson metric
 */
function referringAutoComputedMetrics( cache, namespaceId, lessonMetricName ) {
	const subtree = cache.forest[ namespaceId ];
	const out = [];
	if ( !subtree ) {
		return out;
	}
	for ( const affectedNodes of subtree ) {
		affectedNodes.forEach( ([ id, metric ]) => {
			if ( namespaceId === id && metric.autoCompute && metric.ref === lessonMetricName ) {
				out.push( metric );
			}
		});
	}
...
function coveredLessons( lessons, coverage ) {
	...
	if ( coverageType === 'include' ) {
		keep = lesson => covered.has( lesson._id );
...
function isCoveredLesson( lessonId, coverage ) {
	...
	if ( coverage[ 0 ] === 'include' && coverage.includes( String( lessonId ) ) ) return true;
	if ( coverage[ 0 ] === 'exclude' && !coverage.includes( String( lessonId ) ) ) return true;
	return false;
}
...
async function updateAssessmentDepsCache( cache, metric, entityId, namespaceId ) {
	...
	const referringMetrics = ( namespaceId && isLessonLevel ) ? referringAutoComputedMetrics( cache, namespaceId, metric.name ) : [];
	...
	if ( assessmentDepsCacheNeedsUpdate( cache, metric.level, entityId, metric.name, metric.autoCompute, hasReferringAutoCompute ) ) {
		if ( isLessonLevel ) {
			...
			if ( metric.autoCompute && hasReferringAutoCompute ) {
				...
				const componentMetric = metric.ref;
				const nodes = cache.forest?.[ entityId ][ componentMetric ];
