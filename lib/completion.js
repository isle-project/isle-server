...
const { updateAssessmentsDepCache, CACHE: ASSESSMENT_CACHE } = require( './helpers/completion_cache.js' );
...
router.post( '/create_metric',
	...
	wrapAsync( async function onCreateMetric( req, res ) {
		...
		if ( autoCompute ) {
			updateAssessmentsDepCache( ASSESSMENT_CACHE, level, id, name, autoCompute );
		}
		res.json({
