...
*             schema:
*               type: array
*               description: Array of unique completion component identifiers and their types
*               items:
*                 type: object
*                 properties:
*                   component:
		...
		const lessonComponents = await Completion.aggregate([
			...
			},
			{
				$group: {
					_id: '$lesson',
					components: {
						$addToSet: '$component|%-@|$componentType'
					}
				}
			}
		]);
		const components = lessonComponents.map( lesson => {
			return {
				lesson: lesson._id,
				components: lesson.components.map( component => {
					const [ componentId, componentType ] = component.split( '|%-@|' );
					return {
