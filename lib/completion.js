	...
	wrapAsync( async function onSaveLessonMetrics( req, res ) {
		...
		Promise.all( promises ).then( () => {
			if ( createdLessonPromises.length > 0 ) {
				...
				console.log( '>> CreatedLessons: ', JSON.stringify(createdLessons, null, 2) );
				console.log( '>> ChangedLessons: ', JSON.stringify(changedLessons, null, 2) );
				Promise.all( createdLessonPromises ).then( () => {
					const query = { _id: { $in: changedLessons.concat( createdLessons ) } };
				...
				console.log( 'Retrieve changed lessons for lessons with ids:', changedLessons );
