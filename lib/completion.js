...
const { makeAssessmentDepsCache,
		removeAssessmentDeps,
		updateAssessmentDepsCache } = require( './helpers/completion_cache.js' );
...
router.post( '/create_metric',
	...
	wrapAsync( async function onCreateMetric( req, res ) {
		...
		if ( autoCompute ) {
			updateAssessmentDepsCache( ASSESSMENT_CACHE, level, id, name, autoCompute );
		}
		res.json({
...
/**
 * Removes a users completions from the database for a given metric.
 *
 ...
 * @returns {Promise} resolves when the completions are removed
 */
async function clearUserCompletions( level, id, name ) {
	const metricKey = `${level}-${id}-${name}`;
	const out = await User.updateMany({ [`completions.${metricKey}`]: {
		$exists: true
	}}, {
		$unset: {
