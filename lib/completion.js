...
router.post( '/compute_completions',
	...
	wrapAsync( async function onComputeCompletions( req, res ) {
		const { ids, metric, users, policyOptions } = req.body;
                ...
                const metricName = effectiveMetric[ 0 ].name;

                // ATTN:TODO: Spawn child process to compute completions in order to avoid blocking the server
                const promises = effectiveMetric.map( metric => {
                        return computeCompletions(ids, effectiveMetric, users, policyOptions);
                });
                ...
                Promise.all( promises ).then( async ( completions ) => {
                    users.forEach(user => {
                        ids.forEach((id, index) => {
                            const ukey = `${effectiveMetric[index].level}-${id}-${metricName}`;
                            operations.push({
                                updateOne: {
                                    filter: {
                                        _id: ObjectId(user)
                                    ...
                                    update: {
                                        $set: {
                                            [`completions.${ukey}`]: {
                                                level: effectiveMetric[index].level,
                                                entityId: id,
                                                metricName: metricName,
                                                lastUpdated: String(currentTime),
                                                score: completions[index][user]
                                            }
                                        }
                                    },
                                    upsert: true
	...
	wrapAsync( async function onSaveLessonMetrics( req, res ) {
		...
		for ( let i = 0; i < lessonIDs.length; i++ ) {
			...
			if ( lessonMetrics[ id ] === null ) {
				...
				const metric = new CompletionMetric( lessonMetrics[ id ] );
				console.log( metric );
		...
		Promise.all( promises ).then( () => {
			if ( createdLessonPromises.length > 0 ) {
				...
				Promise.all( createdLessonPromises ).then( () => {
					...
					const onSuccess = lessons => {
						res.json({
							...
							lessons: { changed, created, deleted }
						});
					};
				...
				Lesson.find({ _id: { $in: changed.concat( deleted ) }} ).then( lessons => {
					const idsToLessons = {};
