...
const options = {
	...
	getUser: async ( attributes ) => {  // eslint-disable-line require-await
		...
		console.log( 'Found user, returning JSON Web Token for user authentication...' );
...
function registerSAML( app ) {
	let out;
	if ( !config.saml || config.saml === 'disabled' ) {
		console.log( 'SAML authentication is disabled.' );
		return;
	}
	// Case:  'enabled', 'passive'
	console.log( 'Registering SAML authentication strategy...' );
	...
	if ( config.saml === 'passive' ) {
		console.log( 'Only service provider is exposed...' );
		out = samlTrivialFactory( samlAudiences, options );
	}
	else if ( samlAudiences.length > 1 ) {
		// Multiple audiences:
