...
const options = {
	...
	choiceExtraStyles,
	altLogin: `${SERVER_HOST_NAME}/dashboard/login`,
...
function registerSAML( app ) {
	...
	if ( settings.saml === 'passive' ) {
		...
		out = samlFactory({ ...options, ...samlAudiences[ 0 ] });
	}
	else {
		// No audiences, i.e. no SAML authentication, but still support sign-in barrier via username and password:
		out = samlFactory( [], options );
	}
