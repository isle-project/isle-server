...
const options = {
	...
	getUser: async ( attributes ) => {  // eslint-disable-line require-await
		console.log( 'Get user for attributes: '+JSON.stringify( attributes ) );
...
function registerSAML( app ) {
	let out;
	if ( samlAudiences.length === 0 ) {
		return;
	}
	const { samlFactory, samlMultiFactory } = require( 'express-saml2-middleware' );
