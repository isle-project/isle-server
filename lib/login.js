 ...
 * /ensure_session:
 *   post:
 *     summary: Ensure user session.
 *     description: Ensure a user session is established.
 *     tags: [Users]
 *     security:
 *       - JWT: []
 *     responses:
...
router.post( '/ensure_session',
	...
	function onEnsureSession( req, res ) {
		req.session.loggedIn = true;
		res.json({ message: 'ok' });
	}
);
 ...
 *                   example: ok
 */
router.post( '/invalidate_session',
	function onInvalidateSession( req, res ) {
		req.session = null;
		res.json({ message: 'ok' });
	}
);
 ...
 *               name:
 *                 type: string
 *                 description: Full name of the user (only used when parts of the name are not specified).
 *                 example: John Doe
 *               firstName:
 *                 type: string
	...
	wrapAsync( async function onCompleteRegistration( req, res ) {
		...
		if ( req.body.firstName || req.body.lastName || req.body.preferredName ) {
			...
			if ( req.body.preferredName ) {
				validateString( req.body.preferredName, 'preferredName', req.t );
