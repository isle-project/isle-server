...
NamespaceSchema.path( 'title' ).validate( function validate( title, clbk ) {
	Namespace.findOne( { title: title }, function find( err, ns ) {
		if ( !ns ){
			return clbk( true );
		}
		return clbk( false );
	});
}, 'Namespace with the given title already exists' );
