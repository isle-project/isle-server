...
app.post( '/resend_confirm_email', wrapAsync( async function onResend( req, res ) {
	try {
...
app.post( '/confirm_email', wrapAsync( async function onConfirm( req, res ) {
	...
	const out = { message: res.t('email-confirmation-success') };
	res.json( out );
}) );
...
app.get( '/shibboleth', wrapAsync( async function onLogin( req, res ) {
		let { eppn, name, affil, time, salt, token } = req.query;
		eppn = decodeBase64String( eppn );
		name = decodeBase64String( name );
		affil = decodeBase64String( affil );

		time = roundn( Number( time ), 1 );
		currentTime = roundn( new Date().getTime() / 1000, 1 );
		if ( currentTime - time >= tolerance ) {
...
app.post( '/shibboleth',
	wrapAsync( async function onLogin( req, res ) {
		const { eppn, name, affil, time, salt, token } = req.body;
