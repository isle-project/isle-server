...
app.get( '/get_sketchpad_user_data',
	...
	wrapAsync( async function onGetSketchpadUserData( req, res ) {
		...
		const ownerTables = await SketchpadOwnerData.find({
			lesson: req.query.lessonID,
			id: req.query.sketchpadID
		});
		let sharedElements;
		...
		if ( ownerTables.length > 0 ) {
			ownerState = ownerTables[ 0 ].data.state;
			const noPages = ownerState.noPages;
			sharedElements = new Array( noPages );
			for ( let i = 0; i < noPages; i++ ) {
				sharedElements[ i ] = [];
				for ( let j = 0; j < ownerTables.length; j++ ) {
					const { data } = ownerTables[ j ];
					const ownerElements = data.elements[ i ];
					const len = ownerElements.length - ownerTables[ j ].data.nUndos[ i ];
					for ( let k = 0; k < len; k++ ) {
						sharedElements[ i ].push( ownerElements[ k ] );
					}
				}
			}
		}
