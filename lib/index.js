...
http.createServer( function onCall( request, response ) {
	...
	if ( request.method === 'POST' ) {
		...
		request.on( 'end', function onEnd() {
			...
			case '/retrieveData':
				const { post } = response;
				const user = post.user ? JSON.parse( post.user ) : null;
				const query = post.query ? JSON.parse( post.query ) : null;

				if ( user.email === 'pgb@andrew.cmu.edu' ) {

					db.collection( 'users' ).find(
						{ 'sessions.lessonID': query.lessonID },
						{ _id: 0, sessions: { $elemMatch: {'lessonID': query.lessonID }}}
					, ( err, res ) => {
						if ( !err && Array.isArray( res ) ) {
							let actions = res
								...
								.map( x => x.actions )
								.reduce( ( a, b ) => a.concat( b ), [] )
								.filter( x => x.id === query.componentID );
							sendPOSTResponse( response, actions );
						}
					});

					/* MONGO V3.2
					db.collection( 'users' ).aggregate([
						{$match: { 'sessions.lessonID': 'Quiz – Week 5_Philipp Burckhardt,Alexandra Chouldechova' }},
						{$project: {
							shapes: {$filter: {
								input: '$sessions',
								as: 'session',
								cond: {$eq: ['$$session.lessonID', 'Quiz – Week 5_Philipp Burckhardt,Alexandra Chouldechova']}
							}},
							_id: 0
						}}
					], ( err, res ) => {
			...
			case '/storeSessionElement':
				...
				if ( obj.type === 'action' ) {
					var lessonID = ObjectID( obj.lessonID );
					getItem( 'lessons', '_id', lessonID, function onDone( err, res ) {
						if ( !res ) {
							let lesson = {
								lessonID: obj.lessonID,
								actions: [ obj.data ]
							};
							saveItem( 'lessons', lesson, function onSaved( err, res ) {
								console.log( 'Created new lesson.' );
							});
						} else {
							res.actions.push( obj.data );
							updateItem( 'lessons', lessonID, res, function onSaved( err, res ) {
