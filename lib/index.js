...
app.use( passport.authenticate([ 'administrator' ], { session: false }), express.static( 'logs' ) );
...
passport.use( 'administrator', new JwtStrategy( jwtOptions, function onPayloadReceived( jwtPayload, next ) {
	debug( 'Payload received: ', jwtPayload );
	User.findOne({ '_id': jwtPayload.id, 'administrator': true }, function onFindOne( err, user ) {
		if ( !err ) {
			next( null, user );
		} else {
			next( err, false );
