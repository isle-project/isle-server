...
async function sendCreatedTicketNotification( ticketID, { namespace, user, title, description }) {
	const result = await Namespace.findOne({ _id: namespace }).populate( 'owners', [ 'email', 'name' ]);
	const { owners } = result;
	...
	for ( let i = 0; i < users.length; i++ ) {
		...
		const link = recipient.administrator ?
			...
			`${serverHostName}/dashboard/#/namespace-data/${result.name}/tickets?ticket=${ticketID}`;
		const mail = {
			'from': NOTIFICATIONS_EMAIL,
			'subject': 'New Ticket Created',
			'to': recipient.email,
			'text': `
				Dear ${recipient.name}, the user ${user.name} (${user.email}) taking course ${result.name} has created the following ticket:
				<br />
				<hr />
				<br />
				${title}: <br />
