...
const { middleware: samlMiddleware, router: samlRouter, root: samlRoot } = samlFactory({
	sp: {
	...
	getUser: async (attributes) => ({  // eslint-disable-line require-await
        	'X-Test-Email': attributes['email'] || 'NA',
       		'X-Test-Given': attributes['givenName'] || 'NA',
        	'X-Test-SN':    attributes['sn'] || 'NA',
        	'X-Test-CN':    attributes['cn'] || 'NA ',
    	})
});
