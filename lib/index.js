...
var badgeCheck = require( './badge_check.js' );
...
app.post( '/update_user_session', passport.authenticate( 'jwt', { session: false }), function onUpdateUserSession( req, res ) {
	...
	const user = req.user;
	const { addedScore, elapsed, lessonID, progress, addedChatMessages } = req.body;
	...
	if ( data.addedActionTypes ) {
		if ( !data.actionTypes ) {
			data.actionTypes = {};
		}
		const keys = objectKeys( data.addedActionTypes );
		for ( let i = 0; i < keys.length; i++ ) {
			const type = keys[ i ];
			const count = data.addedActionTypes[ type ];
