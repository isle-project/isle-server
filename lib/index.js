...
function unzipLessonFolder( namespaceName, lessonName, filename ) {
	...
	webshot( server + '/' + namespaceName + '/' + lessonName + '/index.html', dirpath + '/preview.jpg', { renderDelay: 3000 }, function( err ) {
		if ( err ) {
	...
	if ( req.body.stringified ) {
		...
		if ( formData.type === 'action' ) {
			...
			sessionData.save( function( err ) {
				if ( err ) {
					return res.status( 404 ).send( 'Session could not be saved.' );
				}
				res.json({
...
io.on( 'connection', function( socket ) {
	...
	socket.on( 'file_upload', function( data ) {
		debug( 'Should save file...' );
		socket.emit( 'mynews', 'hallo' );
	});
	socket.emit( 'welcome', { 'hallo': 'welt' });
});
