...
app.post( '/update_user_session', passport.authenticate( 'jwt', { session: false }), function onUpdateUserSession( req, res ) {
	...
	user.spentTime += elapsed;
	user.score += addedScore;
	if ( !user.lessonData[ lessonID ] ) {
		user.lessonData[ lessonID ] = {};
	}
	user.lessonData[ lessonID ].progress = progress;
	if ( user.lessonData[ lessonID ].spentTime ) {
		user.lessonData[ lessonID ].spentTime += elapsed;
	} else {
		user.lessonData[ lessonID ].spentTime = elapsed;
	}
	debug( 'New lesson data: '+ JSON.stringify( user.lessonData ) );
	user.save( onSave );
