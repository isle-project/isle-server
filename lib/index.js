...
app.get( '/get_isle_file', function onGetIsleFile( req, res ) {
	var path = namespacesDirectory;
	var namespace = req.query.namespace;
	var lesson = req.query.lesson;
	var file = path + '/' + namespace + '/' + lesson +'/index.isle';

	fs.readFile(file, 'utf8', function readIseFile(err, data) {
		if (err) return res.status( 404 ).send( 'isle file not found.' );
		res.json({
			message: 'ok',
			lessons: data
		});
