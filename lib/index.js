...
const { add } = require('./models/announcement.js');
...
app.post( '/update_lesson',
	...
	wrapAsync( async function onUpdateLesson( req, res ) {
		...
		try {
			...
			if ( lockUntil ) {
				if ( oldEvent && oldEvent.time !== lockUntil ) {
		...
		owners.forEach( owner => {
			...
			if ( !alreadyPresent ) {
				...
				const mail = {
					...
					'to': owner.email,
					'text': `
						Dear ${owner.name}, you were adedd as an instructor to the course "${namespace.title}" at ${organization}.<br />
...
app.post( '/update_cohort',
	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		if ( newProps.members ) {
			// Case: `members` is not an empty string...
			if ( newProps.members.includes( ',' ) ) {
				newProps.members = newProps.members.split( ',' );
			} else {
				newProps.members = [ newProps.members ];
			}
			...
			console.log( `Adding ${addedUsers.length} user(s) to cohort...` );
			...
			console.log( `Inviting ${newEmails.length} new user(s) to cohort...` );
			if ( newEmails.length > 0 ) {
				const cohortStartTime = cohort.startDate.getTime();
				const newUsers = [];
				for ( let i = 0; i < newEmails.length; i++ ) {
