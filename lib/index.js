			...
			case '/retrieveData':
				...
				if ( user.email === 'pgb@andrew.cmu.edu' ) {
					...
					db.collection( 'users' ).find(
						{ 'sessions.lessonID': 'Quiz – Week 5_Philipp Burckhardt,Alexandra Chouldechova' },
						{ _id: 0, sessions: { $elemMatch: {'lessonID': 'Quiz – Week 5_Philipp Burckhardt,Alexandra Chouldechova' }}}
					, ( err, res ) => {
						if ( !err ) {
							let actions = sessions.map( x => x.sessions );
							console.log( actions )
						}
					});
