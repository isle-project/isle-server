...
app.post( '/update_namespace',
	...
	wrapAsync( async function onUpdateNamespace( req, res ) {
		...
		let toRemove = extractOwnersToRemove( newProps.owners, namespace.owners );
		...
		const owners = await User.find({ email: newProps.owners });
		...
		owners.forEach( owner => {
			...
			if ( !alreadyPresent ) {
				debug( `Add namespace to user ${owner.email}.` );
				owner.ownedNamespaces.push( namespace );
				owner.save();
				const mail = {
					'from': NOTIFICATIONS_EMAIL,
					...
					'to': owner.email,
					'text': `
						Dear ${owner.name}, you were adedd as an instructor to the course "${namespace.title}" at ${owner.organization}.<br />
						Please click the link below to login into your ISLE dashboard to view and configure the materials of the course.
					`,
					'link': serverHostName
				};
				console.log( 'Send notification to'+owner.email );
				debug( 'Mail: ' + JSON.stringify( mail ) );
				/*
				mailer.send( mail, function onDone( error, response ) {
					if ( !error ) {
		...
		if ( addedUsers.length > 0 ) {
			for ( let i = 0; i < addedUsers.length; i++ ) {
				...
				const mail = {
					...
					'to': user.email,
					'text': `
						Dear ${user.name}, you were adedd to the course "${cohort.namespace.title}" by ${ownerNames} at ${organization}.<br />
