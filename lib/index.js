...
function recreateShibbolethToken({ eppn, name, affil, time, salt }) {
	...
	console.log( 'Raw token: '+template );
...
function sendVerificationEmail( user ) {
	const mail = {
		...
		'text': `
			...
			<a href="${serverHostName}/dashboard/#/confirm-email/?token=${user._id}">Link</a>
		`
	};
...
function extractEmailsWithoutAccount( memberEmails, existingUsers ) {
	const out = [];
	for ( let i = 0; i < memberEmails.length; i++ ) {
		let found = false;
		for ( let j = 0; j < existingUsers.length; j++ ) {
			if ( existingUsers[ j ].email === memberEmails[ i ] ) {
				found = true;
			}
		}
		if ( !found ) {
...
app.post( '/confirm_email', wrapAsync( async function onConfirm( req, res ) {
	...
	await user.update({ $set: { verifiedEmail: true }});
...
app.post( '/update_cohort',
	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		let users = await User.find({ email: newProps.members });

		const newEmails = extractEmailsWithoutAccount( newProps.members, users );
		if ( newEmails.length > 0 ) {
			const newUsers = [];
			console.log( cohort.namespace );
			const ownerNames = cohort.namespace.owners.map( x => x.name ).join( ', ');
			const organization = ownerNames[ 0 ].organization;
			for ( let i = 0; i < newEmails.length; i++ ) {
				...
				const mail = {
					...
					'to': user.email,
					'text': `
						Dear ${user.name}, you are invited to join the course ${cohort.namespace.title} by ${ownerNames} at ${organization}.<br />
						Please click on the following link to complete the registration processing by choosing a password of your liking:<br />
						<a href="${serverHostName}/dashboard/#/new-password/?token=${user._id}">Link</a><br />
		...
		res.json({ message: req.t( 'cohort-updated' ) });
	})
);
