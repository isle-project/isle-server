...
app.get( '/get_lesson_info', function( req, res ) {
	...
	Namespace.findOne({ title: namespaceName }, function( err, namespace ) {
		...
		Lesson.findOne({ namespace: namespace, title: lessonName }, function( err, lesson ) {
			if ( err ) {
				return debug( 'Encountered an error while retrieving lesson: ' + err.message );
			}
			const info = {
				lessonID: lesson._id,
...
function unzipLessonFolder( namespaceName, lessonName, filename ) {
	...
	let lessonURL = server + '/' + namespaceName + '/' + lessonName + '/index.html';
	debug( `Taking screenshot of ${lessonURL}` );
	webshot( lessonURL, dirpath + '/preview.jpg', { renderDelay: 10000 }, function( err ) {
		if ( err ) {
			debug( 'Encountered error while taking screenshot: ' + err.message );
		}
	});
	fs.unlink( filePath );
}
	...
	User.findOne({ email: req.query.email}, function( err, user ) {
		...
		const mail = {
			...
			'html': `
				...
				<a href="${server}/dashboard/#/new-password#${user._id}">Link</a>
			`
		};
