...
async function sendTicketMessageNotification( ticket, sender, message ) {
	...
	if ( ticket.user.email !== sender.email ) {
		...
		const mail = {
			...
			'text': `
				...
				Please do <b>not</b> reply to this email but click on the link below to respond on the ISLE dashboard.
			`,
			'link': `${serverHostName}/dashboard/#/profile?ticket=${ticketID}`
		};
		debug( 'Mail: ' + JSON.stringify( mail ) );
		mailer.send( mail, function onDone( error ) {
			if ( error ) {
	...
	tickets.messages.forEach( x => {
		if ( !emails.has( x.email ) ) {
			emails.add( x.email );
			recipients.push({
				email: x.email,
				name: x.author
	...
	for ( let i = 0; i < recipients.length; i++ ) {
		...
		const link = recipient.administrator ?
			`${serverHostName}/dashboard/#/admin/tickets?ticket=${ticketID}` :
			`${serverHostName}/dashboard/#/namespace-data/${ticket.namespace.title}/tickets?ticket=${ticketID}`;
		const mail = {
			'from': NOTIFICATIONS_EMAIL,
			'subject': 'New Reply to Ticket: '+ticket.title,
			'to': recipient.email,
			'text': `
				Dear ${recipient.name}, the user <b>${user.name} (${user.email})</b> taking course <b>${result.title}</b> has replied to ticket <b>${ticket.title}</b>:
				<br />
				<br />
				${message}
				<br />
