...
const Session = require( './session.js' ); // eslint-disable-line
const SessionData = require( './session_data.js' );
const SketchpadUserData = require( './sketchpad_user_data.js' );
const SketchpadOwnerData = require( './sketchpad_owner_data.js' );
...
const badgeCheck = require( './badge_check.js' );
...
function check( str ) {
	var name = swot.getInstitutionName( str );
	return name || 'Other';
}

function isOwner( user, namespaceID, clbk ) {
	Namespace.findOne({ _id: namespaceID }, onFindNamespace );
...
app.get( '/delete_lesson', passport.authenticate( 'jwt', { session: false }), function onDeleteLesson( req, res ) {
	...
	function onFindNamespace( err, namespace ) {
		...
		deleteLessonDirectory( join( namespaceName, lessonName ), function onDelete( err ) {
			if ( err ) {
				return res.status( 403 ).send( err.message );
			}
...
app.post( '/new_announcement', passport.authenticate( 'jwt', { session: false }), function onUpdateLesson( req, res ) {
	...
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		...
		namespace.save( onNamespaceUpdate );
	});
	function onNamespaceUpdate( err ) {
		if ( err ) {
			return res.status( 404 ).send( 'Namespace update failed.' );
		}
		res.json({ message: 'Announcement successfully added.' });
	}
});

app.post( '/edit_announcement', passport.authenticate( 'jwt', { session: false }), function onUpdateLesson( req, res ) {
	const { namespaceName, announcement } = req.body;
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		if ( err ) {
			return debug( 'Encountered an error in namespace query: ' + err.message );
...
app.post( '/delete_announcement', passport.authenticate( 'jwt', { session: false }), function onUpdateLesson( req, res ) {
...
app.get( '/activate_lesson', passport.authenticate( 'jwt', { session: false }), function onActivateLesson( req, res ) {
	...
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		...
		Lesson.updateOne({ namespace: namespace, title: lessonName }, { active: true }, function onLessonUpdate( err ) {
			if ( err ) {
				return res.status( 404 ).send( 'Lesson update failed.' );
			}
			res.json({
...
app.get( '/deactivate_lesson', passport.authenticate( 'jwt', { session: false }), function onDeactivateLesson( req, res ) {
	...
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		...
		Lesson.updateOne({ namespace: namespace, title: lessonName }, { active: false }, function onLessonUpdate( err ) {
			if ( err ) {
				return res.status( 404 ).send( 'Lesson update failed.' );
			}
			res.json({
...
app.get( '/show_lesson', passport.authenticate( 'jwt', { session: false }), function onShowLesson( req, res ) {
	...
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		...
		Lesson.updateOne({ namespace: namespace, title: lessonName }, { public: true }, function onLessonUpdate( err ) {
			if ( err ) {
				return res.status( 404 ).send( 'Lesson update failed.' );
			}
			res.json({
...
app.get( '/hide_lesson', passport.authenticate( 'jwt', { session: false }), function onHideLesson( req, res ) {
	...
	Namespace.findOne({ title: namespaceName, owners: { $in: [ req.user ] } }, function onFindNamespace( err, namespace ) {
		...
		Lesson.updateOne({ namespace: namespace, title: lessonName }, { public: false }, function onLessonUpdate( err ) {
			if ( err ) {
				return res.status( 404 ).send( 'Lesson update failed.' );
			}
			res.json({
...
app.post( '/get_namespace_actions', passport.authenticate( 'jwt', { session: false }), function onGetUserActions( req, res ) {
	if ( !req.body.namespaceID ) {
		return res.status( 422 ).send( 'Missing namespaceID.' );
	}
	Lesson.find({ namespace: req.body.namespaceID }, onFindLessons );

	function onFindLessons( err, lessons ) {
		if ( err ) {
			return res.status( 404 ).send( 'Lessons could not be retrieved.' );
		}
		async.map( lessons, mapLesson, function onMapAsync( err, results ) {
			res.json( results );
		});
		function mapLesson( lesson, done ) {
			SessionData
...
app.get( '/get_sketchpad_user_data', passport.authenticate( 'jwt', { session: false }), function onGetSketchpadUserData( req, res ) {
	console.log( req.query );
	isOwner( req.user, req.query.namespaceID, ( owner ) => {
		console.log( owner ? 'User is an owner' : 'User is not an owner' );
...
app.post( '/save_sketchpad_data', passport.authenticate( 'jwt', { session: false }), function onGetSketchpadUserData( req, res ) {
	console.log( 'Should save sketchpad data...' );
	isOwner( req.user, req.body.namespaceID, ( owner ) => {
		if ( owner ) {
			console.log( 'Save sketchpad for owner...' );
			SketchpadOwnerData.findOneAndUpdate(
				{
					lesson: req.body.lessonID,
			...
			function onDone( err, val ) {
				...
				console.log( 'Return message to owner' );
				console.log( val );
				res.json({ message: 'Owner data successfully saved.' });
			});
		} else {
			console.log( 'Save sketchpad for user...' );
			SketchpadUserData.findOneAndUpdate({
				lesson: req.body.lessonID,
				id: req.body.sketchpadID,
				user: req.user
			},
			...
			function onDone( err, val ) {
				...
				console.log( 'Return message to user' );
...
app.get( '/get_enrollable_cohorts', passport.authenticate( 'jwt', { session: false }), function onCohorts( req, res ) {
	Cohort
		.find({
			startDate: { '$lt': new Date() },
			endDate: { '$gte': new Date() },
		...
		.exec( function onPopulatedCohorts( err, cohorts ) {
			if ( err ) {
...
app.get( '/get_cohorts', passport.authenticate( 'jwt', { session: false }), function onCohorts( req, res ) {
	...
	Cohort
		...
		.exec( function onFindCohort( err, cohorts ) {
			if ( err ) {
				return res.status( 404 ).send( 'No cohorts found.' );
			}
			res.json({ message: 'ok', cohorts: cohorts });
...
app.get( '/add_to_cohort', passport.authenticate( 'jwt', { session: false }), function onUpdateCohort( req, res ) {
	...
	Cohort.findOneAndUpdate(
		...
		{ $push: { members: user }},
		{ new: true },
	function onDone( err, cohort ) {
		if ( err ) {
			debug( 'Encountered an error when adding user to cohort: ' + err.message );
			return res.status( 404 ).send( err.message );
		}
		debug( 'Updated cohort...' );
		debug( cohort );
...
app.post( '/update_cohort', passport.authenticate( 'jwt', { session: false }), function onUpdateCohort( req, res ) {
	...
	function onFindCohort( err, cohort ) {
		...
		let newProps = pick( updatedCohort, [ 'members', 'title', 'startDate', 'endDate', 'private', 'emailFilter' ]);
...
app.get( '/delete_file', passport.authenticate( 'jwt', { session: false }), function onDeleteFile( req, res ) {
	File.findOne({ _id: req.query._id }, function onFindCohort( err, file ) {
		if ( err || !file ) {
			debug( 'Encountered an error when deleting file: ' + err );
			return res.status( 404 ).send( 'File does not exist.' );
		}
		file.remove( function onRemoveFile( err ) {
			if ( err ) {
				return res.status( 400 ).send( 'File could not be removed' );
			}
...
app.get( '/get_user_badges', passport.authenticate( 'jwt', { session: false }), function onBadges( req, res ) {
