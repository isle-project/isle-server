...
const storage = multer.diskStorage({
	destination: function onDestination( req, file, cb ) {
		...
		if ( body.owner === 'true' && body.namespaceName ) {
			const namespaceDir = join( MEDIA_DIRECTORY, body.namespaceName );
			exists( namespaceDir, ( bool ) => {
				if ( bool ) {
					console.log( 'Directory already exists, just save file...' );
					return cb( null, namespaceDir );
				}
				console.log( `Create ${MEDIA_DIRECTORY} directory...` );
				fs.mkdir( namespaceDir, () => {
					cb( null, namespaceDir );
				});
			});
		}
