...
app.post( '/update_namespace',
	...
	wrapAsync( async function onUpdateNamespace( req, res ) {
		...
		for ( let i = 0; i < namespace.owners.length; i++ ) {
			...
			user.ownedNamespaces = arr;
			await user.save( onSave );
			toRemove -= 1;
			if ( toRemove === 0 ) {
				const users = await User.find({ email: newProps.owners });
				...
				users.forEach( user => {
					...
					if ( !alreadyPresent ) {
						...
						const mail = {
							...
							'to': user.email,
							'text': `
								Dear ${user.name}, you were adedd as an instructor to the course "${cohort.namespace.title}" at ${organization}.<br />
				...
				await namespace.update( { $set: newProps });
				await renameDirectory( namespace.title, ns.title );
