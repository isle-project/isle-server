...
const {
	backupDirectory, namespacesDirectory, mediaDirectory, localesDirectory, logsDirectory, server: serverHostName
} = config;
...
app.post( '/create_backup',
	...
	wrapAsync( async function onCreateBackup( req, res ) {
		...
		const output = fs.createWriteStream( BACKUP_DIRECTORY, 'test.zip' );
		const archive = archiver( 'zip', {
			zlib: { level: 9 }
		});
		archive.directory( join( __dirname, '..', 'bin' ), 'bin' );
		archive.pipe( output );
		res.json({ message: 'ok' });
	})
);
