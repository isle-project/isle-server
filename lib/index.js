...
function extractExtraEmails( memberEmails, users ) {
	const out = [];
...
app.post( '/update_cohort',
	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		const addedEmails = extractExtraEmails( newProps.members, cohort.members );
			...
			for ( let i = 0; i < addedEmails.length; i++ ) {
				const mail = {
					...
					'text': `
						...
						Please login to your ISLE dashboard to review the materials of the course.
					`,
					'link': `${serverHostName}/dashboard`
				};
				debug( 'Mail: ' + JSON.stringify( mail ) );
				mailer.send( mail, function onDone( error, response ) {
					if ( !error ) {
		...
		const newEmails = extractExtraEmails( newProps.members, users );
