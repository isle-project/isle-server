		...
		function onFindUsers( err, users ) {
			users.forEach( user => {
				...
				for ( let i = 0; i < user.ownedNamespaces.length; i++ ) {
					if ( user.ownedNamespaces[ i ]._id !== namespace._id ) {
	...
	Namespace
		...
		.exec( function onFindNamespace( err, namespace ) {
			...
			namespace.owners.forEach( user => {
				debug( `Removing namespace for user ${user.email}` );
				const arr = [];
				console.log( namespace._id );
				for ( let i = 0; i < user.ownedNamespaces.length; i++ ) {
					console.log( user.ownedNamespaces[ i ]._id );
					console.log( user.ownedNamespaces[ i ] )
					if ( user.ownedNamespaces[ i ]._id !== namespace._id ) {
						arr.push( user.ownedNamespaces[ i ] );
					}
				}
