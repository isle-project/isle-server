...
async function removeOwnedNamespaceFromUsers( namespace, users ) {
	for ( let i = 0; i < users.length; i++ ) {
		const user = users[ i ];
		console.log( `Removing namespace ${namespace.title} (id: ${namespace._id}) for user ${user.email}...` );
		const arr = [];
		for ( let j = 0; j < user.ownedNamespaces.length; j++ ) {
			const ownedNamespace = user.ownedNamespaces[ j ];
			console.log( `Checking namespace ${namespace.title} (id: ${ownedNamespace._id})...` );
			if ( !ownedNamespace._id.equals( namespace._id ) ) {
				arr.push( ownedNamespace );
			}
			else {
				console.log( `Namespace is removed for user ${user.email}...` );
			}
		}
...
app.post( '/delete_namespace',
	...
	wrapAsync( async function onDeleteNamespace( req, res ) {
		...
		console.log( `Deleting namespace from ${users.length} users...` );
		console.log( users );
		console.log( namespace.owners );
