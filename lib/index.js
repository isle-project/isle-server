...
async function sendCohortInvitations( memberEmails, cohort, namespace ) {
	if ( memberEmails.length > 0 ) {
		...
		if ( addedUsers.length > 0 ) {
			...
			for ( let i = 0; i < addedUsers.length; i++ ) {
				...
				const mail = {
					...
					'to': user.email,
					'text': `
						Dear ${user.name}, you were adedd to the course "${namespace.title}" by ${ownerNames} at ${organization}.<br />
		...
		newEmails = extractEmailsWithoutAccount( memberEmails, users );
		...
		if ( newEmails.length > 0 ) {
			...
			for ( let i = 0; i < newEmails.length; i++ ) {
				...
				const mail = {
					...
					'to': user.email,
					'text': `
						Dear ${user.name}, you are invited to join the course "${namespace.title}" by ${ownerNames} at ${organization}.<br />
...
app.post( '/create_cohort',
	...
	wrapAsync( async function onCreateCohort( req, res ) {
		...
		if ( req.body.title && req.body.namespaceID ) {
			...
			const { users, newEmails } = await sendCohortInvitations( students, cohort, namespace );
...
app.post( '/update_cohort',
	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		const { users, newEmails } = await sendCohortInvitations( newProps.members, cohort, cohort.namespace );
