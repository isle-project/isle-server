...
app.post( '/upload_profile_pic', fileUpload.single( 'file' ), passport.authenticate( 'jwt', { session: false }), function onUploadFile( req, res ) {
	...
	function onSaveUser( err ) {
		...
		res.json({ message: 'Profile picture successfully saved.', path: req.file.path });
	}
});
