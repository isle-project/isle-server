	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		if ( newEmails.length > 0 ) {
			...
			for ( let i = 0; i < newEmails.length; i++ ) {
				const user = new User({
					name: newEmails.split( '@' )[ 0 ],
					email: newEmails[ i ],
					organization: institutionName( newEmails[ i ] ),
				});
				newUsers.push( user );
				...
				const mail = {
					...
					'to': user.email,
					'text': `
						Dear ${user.name}, you are invited to join the course ${cohort.namespace.title} by ${ownerNames} at ${organization}.
						Please click on the following link to complete the registration processing by choosing a password of your liking:
						<a href="${serverHostName}/dashboard/#/new-password/?token=${user._id}">Link</a>
