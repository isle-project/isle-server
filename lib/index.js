...
app.post( '/update_user_session', passport.authenticate( 'jwt', { session: false }), function onUpdateUserSession( req, res ) {
	...
	const user = req.user;
	user.spentTime += req.body.elapsed;
	user.score += req.body.addedScore;
	if ( !user.lessonData[ req.body.lessonID ] ) {
		user.lessonData[ req.body.lessonID ] = {};
	}
	user.lessonData[ req.body.lessonID ].progress = req.body.progress;
	user.save();
	res.json({
		score: user.score,
		spentTime: user.spentTime
