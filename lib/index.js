...
app.post( '/create_backup',
	...
	wrapAsync( async function onCreateBackup( req, res ) {
		...
		const args = [ '--db', 'ISLE', '--out', BACKUP_DIRECTORY ];
		const dump = spawn( '/usr/local/bin/mongodump', args );
		...
		dump.on( 'exit', function onExit( code ) {
			...
			const currentDate = new Date();
			const filename = `backup_${currentDate.toLocaleDateString()}_{${currentDate.getTime()}.zip`;
			...
			archive.on( 'warning', function onWarning( err ) {
				console.log( 'Encountered a warning: '+err.message );
			});

			archive.pipe( output );
			archive.directory( join( BACKUP_DIRECTORY, 'dump' ), 'dump' );
