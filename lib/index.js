...
const serveStatic = require( './serve_static.js' );
...
var storage = multer.diskStorage({
	destination: function onDestination( req, file, cb ) {
		...
		if ( body.owner && body.namespaceName ) {
			const dirname = 'media/'+body.owner;
			const dirpath = resolve( __dirname, join( './../media', body.owner ) );
			exists( dirpath, ( bool ) => {
				if ( bool ) {
...
app.get( '/user_update_check', passport.authenticate( 'jwt', { session: false }), function onUserCheck( req, res ) {
	...
	User.findOne({ '_id': id }, ( err, user ) => {
		...
		const hasMostRecent = Date.parse( updatedAt ) === user.updatedAt;
		res.json({
			message: `Most recent user data ${ user.hasMostRecent ? 'has' : 'has not'} already been downloaded.`,
