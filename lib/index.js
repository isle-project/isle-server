...
function sendCohortInvitations( memberEmails, cohort, namespace ) {
...
app.post( '/create_cohort',
	...
	wrapAsync( async function onCreateCohort( req, res ) {
		...
		if ( req.body.title && req.body.namespaceID ) {
			...
			const { users, newEmails } = sendCohortInvitations( students, cohort, namespace );
...
app.post( '/update_cohort',
	...
	wrapAsync( async function onUpdateCohort( req, res ) {
		...
		const { users, newEmails } = sendCohortInvitations( newProps.members, cohort, cohort.namespace );
