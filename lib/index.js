...
const { createWriteStream, readFileSync, rename } = require( 'fs' );
const { readFile, mkdir, rmdir, stat, unlink } = require( 'fs/promises' );
...
app.post( '/login_tfa',
	wrapAsync( async function onLoginTFA( req, res ) {
		...
		if ( correctPassword && tokenVerified ) {
			...
			res.status( 401 ).send( req.t( 'token-incorrect' ) );
		} else {
...
app.post( '/create_backup',
	passport.authenticate( 'jwt', { session: false }),
	wrapAsync( async function onCreateBackup( req, res ) {
		if ( !req.user.administrator ) {
			throw new ErrorStatus( 403, req.t( 'access-denied' ) );
		}
		...
		dump.on( 'exit', function onExit( code ) {
			...
			output.on( 'close', async function onClose() {
				...
				const mail = {
					...
					'to': req.user.email,
					'text': `
						Dear ${req.user.name}, the backup you requested is now available for download from the ISLE dashboard.
					`,
					'link': serverHostName
				};
			...
			archive.on( 'warning', function onWarning( err ) {
...
app.post( '/enable_tfa',
	...
	wrapAsync( async function onEnableTfa( req, res ) {
		...
		if ( verified ) {
			...
			await req.user.save();
			res.json({ message: 'success', verified });
		}
		res.json({ message: 'failure', verified });
	})
);

app.post( '/disable_tfa',
	passport.authenticate( 'jwt', { session: false }),
