...
app.post( '/credentials', passport.authenticate( 'jwt', { session: false }), function onCredentials( req, res ) {
	...
	User
		...
		.exec( function onFindUser( err, user ) {
			...
			console.log( user.ownedNamespaces[ 0 ] );
			res.json({
				id: req.body.id,
				email: user.email,
				name: user.name,
				organization: user.organization,
				enrolledNamespaces: user.enrolledNamespaces.map( namespace => {
					namespace.owners = namespace.owners.map( pickMail );
					return namespace;
				}),
				ownedNamespaces: user.ownedNamespaces.map( namespace => {
					namespace.owners = namespace.owners.map( pickMail );
					return namespace;
				}),
...
app.get( '/delete_namespace', passport.authenticate( 'jwt', { session: false }), function onDeleteNamespace( req, res ) {
	...
	function onFindNamespace( err, namespace ) {
		...
		function onFindUsers( err, users ) {
			users.forEach( user => {
				const idx = user.ownedNamespaces.indexOf( namespace );
				if ( idx !== -1 ) {
					user.ownedNamespaces.splice( idx, 1 );
					user.save();
				}
			});
		}
