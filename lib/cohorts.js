...
router.get( '/get_cohorts',
	passport.authenticate( 'jwt', { session: false }),
	wrapAsync( async function onCohorts( req, res ) {
		const memberFields = req.query.memberFields || 'email name firstName lastName preferredName picture score spentTime lessonData lessonGrades badges anonEmail anonName customFields';
...
router.get( '/get_all_cohorts',
	...
	wrapAsync( async function onGetAllCohorts( req, res ) {
		const memberFields = req.query.memberFields || 'email name firstName lastName preferredName picture';
