...
router.get( '/get_cohorts',
	passport.authenticate( 'jwt', { session: false }),
	wrapAsync( async function onCohorts( req, res ) {
		let memberFields = req.query.memberFields || 'email name picture score spentTime lessonData lessonGrades badges anonEmail anonName assessments customFields';
		validateString( memberFields, 'memberFields', req.t );
		validateObjectId( req.query.namespaceID, 'namespaceID', req.t );
		if ( contains( memberFields, 'name' ) ) {
