...
const { isCustomTag } = require( './helpers/assessments.js' );
 ...
 *               tag:
 *                 type: string
 *                 description: A assessment category to associate with this lesson.
 *                 example: "exam"
 *     responses:
 *       200:
...
router.post( '/create_lesson',
	...
	wrapAsync( async function onCreateLesson( req, res ) {
		const { namespaceName, lessonName, description, metadata, showInGallery, active, tag } = req.body;
		...
		if ( !lesson ) {
			...
			if ( isCustomTag( tag, true ) ) {
				lesson.tag = isWhitespace( tag ) ? void 0 : tag;
			}
			lesson.save({
				timestamps: true
			});
...
router.post( '/update_lesson',
	...
	wrapAsync( async function onUpdateLesson( req, res ) {
		const { namespaceName, lessonName, newTitle, newDescription, lockUntil, lockAfter, hideFromDashboard, template, tag } = req.body;
