	...
	wrapAsync( async function onCreateLesson( req, res ) {
		...
		if ( !lesson ) {
			...
			if ( !namespace.lessons || namespace.lessons.length === 0 ) {
				console.log( 'Attach all lessons to namespace...' );
				const lessons = await Lesson.find({ namespace: namespace });
				namespace.lessons = lessons;
			} else {
				console.log( 'Attach new lesson to namespace...' );
...
router.post( '/update_lesson',
	...
	wrapAsync( async function onUpdateLesson( req, res ) {
		...
		validateString( namespaceName, 'namespaceName', req.t );
		validateString( lessonName, 'lessonName', req.t );
		validateString( newTitle, 'newTitle', req.t );
...
router.get( '/get_lesson',
	wrapAsync( async function onGetLesson( req, res ) {
		validateString( req.query.namespaceName, 'namespaceName', req.t );
		validateString( req.query.lessonName, 'lessonName', req.t );
...
router.get( '/get_lessons', wrapAsync( async function onGetLessons( req, res ) {
	...
	if ( !namespace.lessons || namespace.lessons.length === 0 ) {
		console.log( 'Lessons not attached to namespace, do manual query...' );
		lessons = await Lesson
			.find({ namespace: namespace })
			.populate( 'lockUntil', 'time' )
			.exec();
	} else {
		console.log( 'Return lessons that are part of namespace...' );
...
router.post( '/copy_lesson',
	...
	wrapAsync( async function onCopyLesson( req, res ) {
		...
		validateString( source, 'source', req.t );
		validateString( target, 'target', req.t );
		validateString( sourceName, 'sourceName', req.t );
		validateString( targetName, 'targetName', req.t );
