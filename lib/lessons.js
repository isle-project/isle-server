...
router.post( '/copy_namespace_lessons',
	...
	wrapAsync( async function onCopyLessons( req, res ) {
		...
		console.log( 'Copying '+sourceLessons.length+' lessons to target namespace...' );
		const lessons = [];
		for ( let i = 0; i < sourceLessons.length; i++ ) {
			const lessonTitle = sourceLessons[ i ].title;
			lessons.push({
		...
		console.log( inserted.length+' lessons were successfully inserted into the database...' );
