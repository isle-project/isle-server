		...
		socket.on( 'send_collaborative_editing_events', function onEvents({ docId, data }) {
			if ( member ) {
				...
				if ( !isNonNegativeInteger( version ) ) {
					return member.socket.emit( 'sent_collaborative_editing_events', 'invalid version' );
				}
				...
				if ( !result ) {
					member.socket.emit( 'sent_collaborative_editing_events', 'version not current' );
				}
				member.socket.broadcast.emit( 'collaborative_editing_events' );
				return member.socket.emit( 'sent_collaborative_editing_events', { ...data, ...result, id: id: docID });
			}
		});
		...
		socket.on( 'poll_collaborative_editing_events', function onEvents({ docId, data }) {
			if ( member ) {
				...
				if ( !isNonNegativeInteger( version ) || !isNonNegativeInteger( commentVersion ) ) {
					return member.socket.emit( 'polled_collaborative_editing_events', 'invalid version' );
				}
				let inst = getInstance( docId, member );
				let result = inst.getEvents( version, commentVersion );
				if ( result === false ) {
					return member.socket.emit( 'polled_collaborative_editing_events', 'history no longer available' );
				}
