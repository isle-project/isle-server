...
function SocketHandler( io ) {
	io.on( 'connection', function connect( socket ) {
		...
		socket.on( 'join_collaborative_editing', function onJoining( docID, doc ) {
			if ( member ) {
				let inst = getInstance( docID, member, doc );
		...
		socket.on( 'send_collaborative_editing_events', function onEvents({ docID, data }) {
			if ( member ) {
				...
				let result = inst.addEvents( version, steps, data.comment, data.selection, data.clientID );
				if ( !result ) {
