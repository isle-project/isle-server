	...
	io.on( 'connection', function connect( socket ) {
		...
		socket.on( 'send_collaborative_editing_events', function onEvents({ docID, data }) {
			if ( member ) {
				...
				let result = getInstance( docID, member ).addEvents( version, steps, data.comment, data.clientID );
				if ( !result ) {
					member.socket.emit( 'sent_collaborative_editing_events', docID, 'version not current' );
				}
				let msg = { ...data, ...result };
				member.socket.emit( 'sent_collaborative_editing_events', docID, msg );

				result = inst.getEvents( version, inst.comments.version );
				msg = {
				...
				return member.socket.broadcast.emit( 'collaborative_editing_events', docID, msg );
			}
		});
		...
		socket.on( 'poll_collaborative_editing_events', function onEvents({ docID, data }) {
			if ( member ) {
				...
				const wait = new Waiting( inst, member.email, () => {
					...
					member.socket.emit( 'polled_collaborative_editing_events', docID, msg );
				});
