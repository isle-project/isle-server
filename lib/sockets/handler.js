...
function SocketHandler( io ) {
	io.on( 'connection', function connect( socket ) {
		...
		socket.on( 'join_collaborative_editing', function onJoining( docID ) {
			...
			debug( `Return collaborative document with id ${docID} and ${inst.userCount} users (version: ${inst.version})` );
			const json = {
				doc: inst.doc.toJSON(),
		...
		socket.on( 'send_collaborative_editing_events', function onEvents({ docId, data }) {
			...
			return member.socket.emit( 'sent_collaborative_editing_events', {
				result, steps, comments: data.comment
			});
		});
		...
		*/
		socket.on( 'poll_collaborative_editing_events', function onEvents({ docId, data }) {
			...
			let inst = getInstance( docId, member.email );
