...
class Room {
	constructor( _, member, roomName ) {
		...
		this.socketIDs = {};
		this.socketIDs[ member.email ] = [ member.socket.id ];
		debug( 'SOCKET ID: ' );
	...
	*/
	join( member ) {
		...
		if ( this.socketIDs[ member.email ] ) {
			this.socketIDs[ member.email ].push( socket.id );
		} else {
			this.socketIDs[ member.email ] = [ socket.id ];
		}
	...
	*/
	leave( member ) {
		...
		if ( this.socketIDs[ member.email ] ) {
			this.socketIDs[ member.email ] = this.socketIDs[ member.email ].filter( id => {
		...
		if (
			...
			( !this.socketIDs[ member.email ] || this.socketIDs[ member.email ].length === 0 )
		) {
