...
class Room {
	...
	*/
	sendQuestions( socket ) {
		if ( this.questions.length > 0 ) {
			socket.emit( 'open_questions', this.questions );
		}
	}
	...
	*/
	addQuestion( question, member ) {
		...
		member.socket.to( this.name ).emit( 'added_question', this.questions );
	...
	*/
	removeQuestion( question, member ) {
		this.questions = this.questions.filter( x => {
			x.email === question.email && x.value === question.value;
		});
		member.socket.to( this.name ).emit( 'removed_question', this.questions );
		member.socket.emit( 'queue_questions', this.questions );
	}
