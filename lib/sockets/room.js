...
class Room {
	constructor( _, member, roomName ) {
		...
		// List of members who are in the process of leaving:
		this.leavingMembers = [];
	...
	*/
	join( member ) {
		...
		this.leavingMembers = this.leavingMembers.filter( email => {
	...
	/**
	* A member should be kicked out of the room soon unless the member returns.
	*
	* @param {Object} member - member object
	*/
	leaveWithDelay( member ) {
		this.leavingMembers.push( member.email );
		setTimeout( () => {
			debug( 'CHECK whether member should still leave...' );
			if ( contains( this.leavingMembers, member.email ) ) {
				debug( 'User is in list of leaving members: '+this.leavingMembers.join( ', ' ) );
				this.leave( member );
			}
		}, 5000 );
	}
