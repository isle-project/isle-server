...
class Room {
	constructor( _, member, roomName ) {
		...
		this.memberSocketIDs = {};
		this.memberSocketIDs[ member.email ] = [ member.socket.id ];
		debug( 'SOCKET ID: ' );
		console.log( this.memberSocketIDs );
	...
	*/
	join( member ) {
		...
		if ( this.memberSocketIDs[ member.email ] ) {
			this.memberSocketIDs[ member.email ].push( socket.id );
		} else {
			this.memberSocketIDs[ member.email ] = [ socket.id ];
		}
		debug( 'PRINT SOCKET IDS');
		console.log( this.memberSocketIDs );
	...
	*/
	leave( member ) {
		...
		this.memberSocketIDs[ member.email ] = this.memberSocketIDs[ member.email ].filter( id => {
			return id !== member.socket.id;
		});
		debug( 'REMAINING ID AFTER LEAVE' );
		console.log( this.memberSocketIDs );
