...
class Room {
	...
	*/
	join( member ) {
		...
		this.leavingMembers.delete( member.email );
	...
	*/
	leaveWithDelay( member ) {
		this.leavingMembers.add( member.email );
		setTimeout( () => {
			debug( `CHECK whether member ${member.email} should still leave...` );
			if ( this.leavingMembers.has( member.email ) ) {
				debug( 'User is in list of leaving members...' );
				this.leave( member );
			}
		}, 5000 );
	}
